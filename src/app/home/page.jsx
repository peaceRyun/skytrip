...
import { useCurrWeather, useWeather } from '../hooks/useWeather';
...
import { getBaseTime, groupForecastByDate } from '../utils/utils';
...
export default function HomePage() {
    ...
    const baseTime = '1400';
    const currBaseTime = getBaseTime();
    const { data, isLoading } = useWeather({ nx, ny, baseDate, baseTime });
    const { data: Currdata, isLoading: isLoadingCurr } = useCurrWeather({
        nx,
        ny,
        baseDate,
        baseTime: currBaseTime,
    });

    if (isLoading) return <p className='text-white'>날씨 로딩 중...</p>;
    if (!data) return <p className='text-white'>날씨 데이터를 불러오지 못했습니다.</p>;

    if (isLoadingCurr) return <p className='text-white'>현재 날씨 로딩 중...</p>;
    if (!Currdata) return <p className='text-white'>현재 날씨 데이터를 불러오지 못했습니다.</p>;
    ...
    return (
        <main className='min-h-screen bg-gradient-to-b from-sky-400 to-blue-900 text-white px-4 py-6'>
            <div className='max-w-xl mx-auto space-y-8'>
                <CurrentWeather current={current} currdata={Currdata} isLoadingCurr={isLoadingCurr} />
